...
@section('header')
  <h2 class="font-semibold text-xl leading-tight text-gray-900">Editar pago</h2>
@endsection
...
<div class="max-w-3xl mx-auto">
  ...
  <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200">
    ...
    <form action="{{ route('pagos.update',$pago) }}" method="POST" class="px-6 pb-6 pt-5">
      @csrf @method('PUT')
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="md:col-span-2">
          ...
          <select name="miembro_id" required
            ...
            @foreach($miembros as $m)
              <option value="{{ $m->id }}" @selected(old('miembro_id',$pago->miembro_id)==$m->id)>
                {{ $m->nombre }} {{ $m->apellido }}
              </option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <input type="date" name="fecha" value="{{ old('fecha',$pago->fecha->format('Y-m-d')) }}"
        ...
        <div>
          ...
          <input type="number" min="0" step="1" name="monto" value="{{ old('monto',$pago->monto) }}"
                 class="mt-1 block w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" required>
        ...
        <div>
          ...
          <select name="metodo" required
            ...
            @foreach($metodos as $m)
              <option value="{{ $m }}" @selected(old('metodo',$pago->metodo)===$m)>{{ $m }}</option>
            @endforeach
          </select>
        ...
        <div class="md:col-span-2">
          ...
          <input type="text" name="referencia" value="{{ old('referencia',$pago->referencia) }}"
                 class="mt-1 block w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
        </div>
        ...
        <div class="md:col-span-2">
          ...
          <textarea name="notas" rows="3"
                    class="mt-1 block w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">{{ old('notas',$pago->notas) }}</textarea>
        </div>
      </div>
      ...
      <div class="mt-7 flex items-center gap-3">
        ...
        <a href="{{ route('pagos.index') }}" class="text-sm font-semibold text-gray-700 hover:text-gray-900">Cancelar</a>
      </div>
    </form>
  </div>
</div>
